...
void StateGraph::removeState(const std::string name) {
    ...
    std::vector<Transition>::iterator itFrom;
    std::vector<Transition>::iterator itTo;
    for(it = states.begin(); it<states.end();) {
        ...
        if((st.name == name) || (st.name.find(name+".") != string::npos))
        {
            ...
            do{
                ...
                if(itFrom != transitions.end())
                    transitions.erase(itFrom);
            }while(itFrom != transitions.end());

            //Remove all the transitions TO the state

            do{
                itTo=getTransition(st.name,false);
                if(itTo != transitions.end())
                    transitions.erase(itTo);
            }while(itTo != transitions.end());
...
void StateGraph::renameState(const std::string oldName, const std::string newName){
    if( oldName == newName)
        return;
    ...
    State st;
    st.name = newName;
    it = find(states.begin(), states.end(), st);
    ...
    if((*it).type=="composit")
    {
        for(StateItr i=states.begin(); i<states.end();i++)
        {
            State& s=*i;
...
StateGraph::TransitionItr StateGraph::getTransition(const std::string stateName,
                                                    bool from,
                                                    std::vector<std::string> events) {
...
void StateGraph::updateTransitions(const std::string oldName, const std::string newName)
{
    TransitionItr itTrans;

    for(itTrans=transitions.begin();itTrans<transitions.end();itTrans++)
    {
        Transition& transition = *itTrans;
